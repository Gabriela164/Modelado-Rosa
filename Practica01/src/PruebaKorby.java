...
/**
* Clase de la prueba a presentar, donde siemper gana Korby
* @author Juan Daniel San Martin Macias, Gabriela López Diego.
* @version 1.0 Septiembre 2022
*/
public class PruebaKorby extends Pruebas{

	/**
	 * Cosntructor vacio de la clase que genera las pruebas de Korby
	 */
	public PruebaKorby(){
	}

	/**
	 * Método encargado de generar la batalla donde gana Korby
	 */
	...
	public void pelea(){
      ...
      //**********************************************Empieza el combate***********************************************
      //Pelea en la que ganará Korby
      System.out.println("Aparecen los tres Peleadores");
      while(meganman.getVida() > 0 && dittu.getVida() > 0){
         korby.ataque();
         meganman.recibeDanio(korby.getDanio());
         dittu.recibeDanio(korby.getDanio());
         ...
         dittu.ataque();
         korby.recibeDanio(dittu.getDanio());
      ...
      System.out.println("Korby obtuvo el poder del Guión y obtiene +10 de vida.");
      korby.setVida(korby.getVida() + 10);
      System.out.println("Ganó Korby con una vida sobrante de " + korby.getVida());
      combate1.setGanadorDelCombate(korby);
   }
}